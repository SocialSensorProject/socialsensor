...
\end{enumerate}
%In this paper, we focus on ranking metrics since ultimately the output of the topical filters
%will be for user consumption.  
%Because all of our classifiers provide a real-valued score (e.g., logistic regression
%provides the probability of the class), the classifiers can be used to 
...
\item \textit{Logistic Regression and RankSVM}: $L_2$ regularization constant $C$ for both methods is tuned for $C \in \{10^{-12}, 10^{-11}, ..., 10^{11}, 10^{12} \}$.
...
\item \textit{All Classfiers}: The number of top features $M$ selected based on their Mutual Information is tuned for $M \in \{10^{2}, 10^{3}, 10^{4}, 10^{5}, 1166582 \textrm{ (all features) } \}$.
\end{itemize}
...
(AP)~\cite{manning_ir} ranking metric discussed more below. % to select the best scoring function $f^t$ on the validation data.
