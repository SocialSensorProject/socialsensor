 ...
 * and/or infinities) may thus give incorrect results, like multiplications,
 * divisions or functions mapping.
 * </p>
 *
 * @since 2.0
 */
public class OpenMapRealVector extends SparseRealVector implements Serializable {
    ...
    /**
     * Build a 0-length vector. Zero-length vectors may be used to initialized
     * construction of vectors by data gathering. We start with zero-length and
     * use either the {@link
     * #OpenMapRealVector(OpenMapRealVector, int)} constructor or one of the
     * {@code append} method ({@link #append(double)},
    ...
    /**
     * Create from an array, specifying zero tolerance. Only non-zero entries
     * will be stored.
     *
     * @param values Set of values to create from.
     * @param epsilon Tolerance below which a value is considered zero.
    ...
    /**
     * Optimized method to add two OpenMapRealVectors. It copies the larger
     * vector, then iterates over the smaller.
     *
    ...
    public OpenMapRealVector append(OpenMapRealVector v) {
        ...
        while (iter.hasNext()) {
            iter.advance();
            res.setEntry((int) iter.key() + virtualSize, iter.value());
        }
        return res;
    }
    ...
    public OpenMapRealVector ebeMultiply(RealVector v)
        ...
        while (iter.hasNext()) {
            iter.advance();
            res.setEntry((int) iter.key(), iter.value() * v.getEntry((int) iter.key()));
        }
        return res;
    }
    ...
    /**
     * Distance between two vectors. This method computes the distance
     * consistent with L<sub>1</sub> norm, i.e. the sum of the absolute values
    ...
    public double getL1Distance(OpenMapRealVector v)
        ...
        while (iter.hasNext()) {
            iter.advance();
            double delta = FastMath.abs(iter.value() - v.getEntry((int) iter.key()));
            max += delta;
        }
        iter = v.getEntries().iterator();
        while (iter.hasNext()) {
    ...
    public double[] toArray() {
        ...
        while (iter.hasNext()) {
            iter.advance();
            res[(int) iter.key()] = iter.value();
        }
        return res;
    }

    /**
     * {@inheritDoc} Implementation Note: This works on exact values, and as a
     * result it is possible for {@code a.subtract(b)} to be the zero vector,
     * while {@code a.hashCode() != b.hashCode()}.
     */
    @Override
    public int hashCode() {
        final int prime = 31;
    ...
    /**
     * {@inheritDoc} Implementation Note: This performs an exact comparison, and
     * as a result it is possible for {@code a.subtract(b}} to be the zero
     * vector, while {@code  a.equals(b) == false}.
     */
    @Override
    public boolean equals(Object obj) {
        ...
        while (iter.hasNext()) {
            iter.advance();
            double test = other.getEntry((int) iter.key());
        ...
        while (iter.hasNext()) {
            ...
            if (Double.doubleToLongBits(test) != Double.doubleToLongBits(getEntry((int) iter.key()))) {
                return false;
            }
        }
        return true;
    ...
    /**
     * Implementation of {@code Entry} optimized for OpenMap. This
     * implementation does not allow arbitrary calls to {@code setIndex} since
     * the order in which entries are returned is undefined.
     */
    ...
    /**
     * Iterator class to do iteration over just the non-zero elements. This
     * implementation is fail-fast, so cannot be used to modify any zero
     * element.
     */
