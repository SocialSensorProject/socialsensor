function hAxes = dscatter(X,Y, path, fName, titleStr, xLabelStr, yLabelStr, varargin)
...
%xlabel(xLabelStr);
%ylabel(yLabelStr);
%print(gcf,[path,'dscatterPlot_' fName], '-dpdf');
%export_fig [path,'dscatterPlot_' fName]
pubmode('on');
...
pName = [path,'dscatterPlot_' fName];
exportfig(gcf, pName,'Format', 'eps', 'Color', 'rgb')
export_fig(pName, '-png', '-transparent');
%export_fig(pName, '-pdf', '-transparent');

%print(gca,'dscatterFile', '-dpng');
