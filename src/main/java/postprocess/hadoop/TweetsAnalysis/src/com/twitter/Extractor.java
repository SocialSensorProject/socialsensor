package postprocess.hadoop.TweetsAnalysis.src.com.twitter;
...
public class Extractor {
    ...
    public List<Entity> extractMentionsOrListsWithIndices(String text) {
        ...
        List<Entity> extracted = new ArrayList<>();
        Matcher matcher = com.twitter.Regex.VALID_MENTION_OR_LIST.matcher(text);
        while (matcher.find()) {
            String after = text.substring(matcher.end());
            if (!com.twitter.Regex.INVALID_MENTION_MATCH_END.matcher(after).find()) {
                if (matcher.group(com.twitter.Regex.VALID_MENTION_OR_LIST_GROUP_LIST) == null) {
                    extracted.add(new Entity(matcher, Entity.Type.MENTION, com.twitter.Regex.VALID_MENTION_OR_LIST_GROUP_USERNAME));
                } else {
                    extracted.add(new Entity(matcher.start(com.twitter.Regex.VALID_MENTION_OR_LIST_GROUP_USERNAME) - 1,
                            matcher.end(com.twitter.Regex.VALID_MENTION_OR_LIST_GROUP_LIST),
                            matcher.group(com.twitter.Regex.VALID_MENTION_OR_LIST_GROUP_USERNAME),
                            matcher.group(com.twitter.Regex.VALID_MENTION_OR_LIST_GROUP_LIST),
                            Entity.Type.MENTION));
                }
            }
        }
    ...
    public String extractReplyScreenname(String text) {
        ...
        Matcher matcher = com.twitter.Regex.VALID_REPLY.matcher(text);
        if (matcher.find()) {
            String after = text.substring(matcher.end());
            if (com.twitter.Regex.INVALID_MENTION_MATCH_END.matcher(after).find()) {
                return null;
            } else {
                return matcher.group(com.twitter.Regex.VALID_REPLY_GROUP_USERNAME);
            }
        } else {
            return null;
        }
    ...
    public List<Entity> extractURLsWithIndices(String text) {
        ...
        Matcher matcher = com.twitter.Regex.VALID_URL.matcher(text);
        while (matcher.find()) {
            if (matcher.group(com.twitter.Regex.VALID_URL_GROUP_PROTOCOL) == null) {
                ...
                if (!extractURLWithoutProtocol
                        || com.twitter.Regex.INVALID_URL_WITHOUT_PROTOCOL_MATCH_BEGIN
                        .matcher(matcher.group(com.twitter.Regex.VALID_URL_GROUP_BEFORE)).matches()) {
                    continue;
                }
            }
            String url = matcher.group(com.twitter.Regex.VALID_URL_GROUP_URL);
            int start = matcher.start(com.twitter.Regex.VALID_URL_GROUP_URL);
            int end = matcher.end(com.twitter.Regex.VALID_URL_GROUP_URL);
            Matcher tco_matcher = com.twitter.Regex.VALID_TCO_URL.matcher(url);
            if (tco_matcher.find()) {
    ...
    private List<Entity> extractHashtagsWithIndices(String text, boolean checkUrlOverlap) {
        ...
        List<Entity> extracted = new ArrayList<>();
        Matcher matcher = com.twitter.Regex.VALID_HASHTAG.matcher(text);

        while (matcher.find()) {
            String after = text.substring(matcher.end());
            if (!com.twitter.Regex.INVALID_HASHTAG_MATCH_END.matcher(after).find()) {
                extracted.add(new Entity(matcher, Entity.Type.HASHTAG, com.twitter.Regex.VALID_HASHTAG_GROUP_TAG));
            }
        }
    ...
    public List<Entity> extractCashtagsWithIndices(String text) {
        ...
        List<Entity> extracted = new ArrayList<>();
        Matcher matcher = com.twitter.Regex.VALID_CASHTAG.matcher(text);

        while (matcher.find()) {
            extracted.add(new Entity(matcher, Entity.Type.CASHTAG, com.twitter.Regex.VALID_CASHTAG_GROUP_CASHTAG));
        }
