...
public class LogisticRegression extends Predictor {
	...
	private Model LrTrain(){
		...
		prob.y = new int[dataCount]; //use latest liblinear
		prob.l = dataCount;
		//prob.n = getFeaturesZahra(_trainData._data.get(0)).length - 1;
		prob.n = getFeaturesZahra(_trainData._data.get(0)).length + 1;//ZAHRA why did it left out the first two entries? I added features+1bias
		prob.x = new FeatureNode[dataCount][];
		prob.bias = 1;
		for (int i = 0; i < dataCount; i++) {
			double[] features = getFeaturesZahra(_trainData._data.get(i));
	...
	private FeatureNode[] toLiblinearSparseFeaturesFormat(double[] feats){
		...
		FeatureNode[] tfeatures = new FeatureNode[features.size()];//ZAHRA do I need to add the bias feature? (biasIndex, 1)
	...
	public int evaluate(DataEntry de) {
		double prediction;
		double[] features = getFeaturesZahra((DataEntry)de);
	...
	public static void main(String[] args) throws IOException{
		...
		lr.runTests("/Users/zahraiman/University/FriendSensor/SPARK/SocialSensorProject_Aug19_2/socialsensor/ClusterResults/TestTrain_Arff/testTrain_1.arff", 1);
	}
}
