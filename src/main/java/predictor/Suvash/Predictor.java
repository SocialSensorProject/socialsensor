...
public abstract class Predictor {
	...
	public final static int CLASS_INDEX = 0;						//ZAHRA CHANGED from 2 to 0
	...
	public double[] getFeaturesZahra(DataEntry dataEntry){
		double[] features;
		int startInd = 0;
		Integer value;
		features = new double[dataEntry._entries.size()-startInd];
		for(int i = startInd; i < dataEntry._entries.size(); i++){
			if(dataEntry._entries.get(i) instanceof Double){
				features[i-startInd] = (Double) dataEntry._entries.get(i);
			}
			else{
				value = (Integer) dataEntry._entries.get(i);
				features[i-startInd] = value.doubleValue();
			}
		}
