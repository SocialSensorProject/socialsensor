...
import static predictor.de.bwaldvogel.liblinear.Linear.*;


public class Predict {
    private static DecimalFormat df3 = new DecimalFormat("#.###");
    private static boolean       flag_predict_probability = false;
    private static double precision = 0, recall = 0, fscore = 0, accuracy = 0, error = 0;
    ...
    static void doPredict(BufferedReader reader, Writer writer, Model model) throws IOException {
        ...
        while ((line = reader.readLine()) != null) {
            ...
            //ZAHRA ==============================================================
            if (predict_label == target_label && target_label == 1) truePositive++;
            if (predict_label == 1 && target_label == 0) falsePositive++;
            if (predict_label == 0 && target_label == 1) falseNegative++;
            //ZAHRA ==============================================================

            errorTmp += (predict_label - target_label) * (predict_label - target_label);
            sump += predict_label;
            sumt += target_label;
        ...
        //ZAHRA ==============================================================
        precision = truePositive/(double)(truePositive + falsePositive);			// precision over folds
        recall    = truePositive/(double)(truePositive + falseNegative);			// recall over folds														// recall over folds
        accuracy = (double)correct / total;
        if((truePositive + falsePositive) == 0)
            precision = 0;
        if((truePositive + falseNegative) == 0)
            recall = 0;
        fscore =  2d * ((precision * recall)/(double)(precision + recall));
        ...
        if (model.solverType.isSupportVectorRegression()) //
        {
            ...
            info("Precision= " + df3.format(precision) +"%n");
            info("Recall= " + df3.format(recall) +"%n");
            info("F-Score= " + df3.format(fscore) +"%n");
            info("Error= " + df3.format(error) +"%n");
        }
    }
    ...
    public double[] mainPredict(String[] argv) throws IOException {
        int i;
