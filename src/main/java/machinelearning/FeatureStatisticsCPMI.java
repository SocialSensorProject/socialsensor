 ...
 */
public class FeatureStatisticsCPMI {
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        for(groupNum = 3; groupNum <= numOfGroups; groupNum++) {
            ...
            df1 = tweet_user_hashtag_grouped.join(df1, tweet_user_hashtag_grouped.col("username").equalTo(df1.col("C0"))).drop(tweet_user_hashtag_grouped.col("username")).drop(df1.col("C0"));//tid, location
            df1.printSchema();
            df1.select(df1.col("tid"), df1.apply("C1").as("location"), df1.col("hashtag")).coalesce(numPart).write().mode(SaveMode.Overwrite).parquet(dataPath + "tweet_location_hashtag_grouped_parquet");*/
    ...
    */
    ...
    public static void calcTweetCondContainLocationConditionalEntropy(final int groupNum) {
        final double tweetNum = tweetCount;
        StructField[] fields1 = {
                DataTypes.createStructField("username", DataTypes.StringType, true),
                DataTypes.createStructField("prob", DataTypes.DoubleType, true)
