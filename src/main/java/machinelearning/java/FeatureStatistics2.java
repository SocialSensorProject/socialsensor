 ...
 */
public class FeatureStatistics2 {
    ...
    /*
      ...
      * Finds the conditional entropy of tweetContainHashtag | FromUser = true
    */
    ...
    */
    public static void calcTweetCondFromUserConditionalEntropy(final int groupNum){
        ...
        //results2 = sqlContext.createDataFrame(res.coalesce(numPart), new StructType(fields));
...
/*    private static String hdfsPath;
    ...
    public static long[] findContainNotContainTopicalHashtagCount(final int groupNum){
        JavaRDD<Row> containNotContainNum = tweet_user_hashtag_grouped.drop("username").javaRDD().mapToPair(new PairFunction<Row, Integer, Long>() {
            @Override
            public Tuple2<Integer, Long> call(Row row) throws Exception {
                List<String> tH = new ArrayList<String>(Arrays.asList((row.getString(1).split(","))));
        ...
        (sqlContext.createDataFrame(containNotContainNum, new StructType(fields1))).registerTempTable("containNumTable");
        long[] containNotCountainCounts = new long[2];
        containNotCountainCounts[0] = sqlContext.sql("select num from containNumTable where key = 1").head().getLong(0);
        containNotCountainCounts[1] = sqlContext.sql("select num from containNumTable where key = 2").head().getLong(0);
        return containNotCountainCounts;
    }
    public static void calcTweetCondFromUserConditionalEntropy(final int groupNum){
        StructField[] fields = {
                DataTypes.createStructField("username", DataTypes.StringType, true),
        ...
        //TODO should I cache some of the user probabilities in the memory
        long[] containNotCountainCounts = findContainNotContainTopicalHashtagCount(groupNum);
        final double[] probTweetContainOrNot = new double[2];
        probTweetContainOrNot[0] = (double)containNotCountainCounts[0] / tweetCount;
        probTweetContainOrNot[1] = (double)containNotCountainCounts[1] / tweetCount;
        String[] tablePartNames = {"condEntropyTweetTrueFromUserTrue", "condEntropyTweetFalseFromUserTrue"};
        String[] CETableName = {"condEntropyTweetFromUser", "condEntropyTweetToUser"};
        String[] MITableName = {"mutualEntropyTweetFromUser", "mutualEntropyTweetToUser"};
        JavaRDD res = null;JavaRDD[] resMI = null;
        JavaRDD tores = null;JavaRDD[] toresMI = null;
        DataFrame results2, toresults2, fromresults2;
        //==============================================================================================================
        for(int partNum = 0; partNum < 2; partNum++) {
            final int p = partNum;
            results2 = sqlContext.createDataFrame(calcProb(groupNum, true, partNum, tweetCount).coalesce(numPart), new StructType(fields));
            results2.registerTempTable(tablePartNames[partNum]);

            fromresults2 = results2.join(fromUserProb, fromUserProb.col("username1").equalTo(results2.col("username"))).drop("username1");
            if(res == null)
            res = (fromresults2.javaRDD().map(new Function<Row, Row>() {
                @Override
                public Row call(Row row) throws Exception {
                    if (row.getDouble(1) == 0 || row.getDouble(2) == 0)
                        return RowFactory.create(row.getString(0), 0.0);
            ...
            resMI[partNum] = fromresults2.javaRDD().map(new Function<Row, Row>() {
                @Override
                public Row call(Row row) throws Exception {
                    if (probTweetContainOrNot[p] == 0)
                        System.out.println("NOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO");
                    if (row.getDouble(1) == 0 || row.getDouble(2) == 0)
                        return RowFactory.create(row.getString(0), 0.0);
                    else
                        return RowFactory.create(row.getString(0), row.getDouble(1) * Math.log(row.getDouble(1) / (probTweetContainOrNot[p] * row.getDouble(2))));
                }
            });
            toresults2 = results2.join(toUserProb, toUserProb.col("username1").equalTo(results2.col("username"))).drop("username1");
            tores[partNum] = toresults2.javaRDD().map(new Function<Row, Row>() {
                @Override
                public Row call(Row row) throws Exception {
                    if (row.getDouble(1) == 0 || row.getDouble(2) == 0)
                        return RowFactory.create(row.getString(0), 0.0);
            ...
            toresMI[partNum] = toresults2.javaRDD().map(new Function<Row, Row>() {
                @Override
                public Row call(Row row) throws Exception {
                    if(row.getDouble(1) == 0 || row.getDouble(2) == 0)
                        return RowFactory.create(row.getString(0), 0.0);
            ...
            System.out.println("SIZE "+partNum+"=================" + resMI[partNum].count() + "================" );
            System.out.println("SIZE "+partNum+" TO ================="+ toresMI[partNum].count() + "================");
        }
        //==============================================================================================================
        for(int partNum = 0; partNum < 2; partNum++) {
            final int p = partNum;
            results2 = sqlContext.sql("select username, (" + containNotCountainCounts[partNum] + "-(prob*" + BigInteger.valueOf((long) tweetCount) + "))/" +
                    BigInteger.valueOf((long) tweetCount) + " AS prob from "+tablePartNames[partNum]);
            fromresults2 = results2.join(fromUserProb, fromUserProb.col("username1").equalTo(results2.col("username"))).drop("username1");
            res[partNum] = res[partNum].union(fromresults2.javaRDD().map(new Function<Row, Row>() {
                @Override
                public Row call(Row row) throws Exception {
                    if (row.getDouble(1) == 0 || row.getDouble(2) == 0)
                        return RowFactory.create(row.getString(0), 0.0);
            ...
            resMI[partNum] = resMI[partNum].union(fromresults2.javaRDD().map(new Function<Row, Row>() {
                @Override
                public Row call(Row row) throws Exception {
                    if (row.getDouble(1) == 0 || row.getDouble(2) == 0)
                        return RowFactory.create(row.getString(0), 0.0);
                    else
                        return RowFactory.create(row.getString(0), row.getDouble(1) * Math.log(row.getDouble(1) / (probTweetContainOrNot[p] * (1 - row.getDouble(2)))));
                }
            }));
            System.out.println("SIZE 3=================" + resMI[partNum].count() + "================");
            toresults2 = results2.join(toUserProb, toUserProb.col("username1").equalTo(results2.col("username"))).drop("username1");
            tores[partNum] = tores[partNum].union(toresults2.javaRDD().map(new Function<Row, Row>() {
                @Override
                public Row call(Row row) throws Exception {
                    if (row.getDouble(1) == 0 || row.getDouble(2) == 0)
                        return RowFactory.create(row.getString(0), 0.0);
            ...
            toresMI[partNum] = toresMI[partNum].union(toresults2.javaRDD().map(new Function<Row, Row>() {
                @Override
                public Row call(Row row) throws Exception {
                    if (row.getDouble(1) == 0 || row.getDouble(2) == 0)
                        return RowFactory.create(row.getString(0), 0.0);
            ...
            System.out.println("SIZE 3 to =================" + toresMI[partNum].count() + "================");
            sqlContext.createDataFrame(res[partNum].coalesce(numPart), new StructType(fields)).registerTempTable(CETableName[partNum]);
            sqlContext.createDataFrame(resMI[partNum].coalesce(numPart), new StructType(fields)).registerTempTable(MITableName[partNum]);
        }
        //======================== COMPUTE COND ENTROPY=================================================================
        String[] cTableNames = {"CondEntropyTweetFromUser", "CondEntropyTweetToUser"};
        String[] mTableNames = {"mutualEntropyTweetFromUser", "mutualEntropyTweetToUser"};
        for(int partNum = 1; partNum < 2; partNum++) {
            fromresults2 = sqlContext.sql("SELECT username, -sum(prob) AS condEntropy FROM " + CETableName[partNum] + " GROUP BY username");
            fromresults2 = fromresults2.sort(fromresults2.col("condEntropy"));//.limit((int) fromSize / 2 + topUserNum / 2);
            fromresults2.cache();
            output(fromresults2.coalesce(1), cTableNames[partNum], false);

            fromresults2 = sqlContext.sql("SELECT username, sum(prob) AS mutualEntropy FROM "+ MITableName[partNum] + " GROUP BY username");
            fromresults2 = fromresults2.sort(fromresults2.col("mutualEntropy"));
            output(fromresults2.coalesce(1), mTableNames[partNum], false);
        }
    }



    private static JavaRDD<Row> calcProb(final int groupNum, final boolean containFlag, DataFrame df, final double tweetNum){
        final List<String> hashtagList = getGroupHashtagList(groupNum);
        return df.javaRDD().mapToPair(new PairFunction<Row, String, Double>() {
            @Override
            public Tuple2<String, Double> call(Row row) throws Exception {
    ...
    public static void calcTweetCondFromHashtagConditionalEntropy(final int groupNum){
        ...
        String[] CETableName = {"condEntropyTweetFromUser", "condEntropyTweetToUser"};
        String[] MITableName = {"mutualEntropyTweetFromUser", "mutualEntropyTweetToUser"};
        JavaRDD[] res = new JavaRDD[2];JavaRDD[] resMI = new JavaRDD[2];
        JavaRDD[] tores = new JavaRDD[2];JavaRDD[] toresMI = new JavaRDD[2];
        DataFrame results2, toresults2, fromresults2;
        //==============================================================================================================
        for(int partNum = 0; partNum < 2; partNum++) {
            final int p = partNum;
            JavaRDD<Row> condEntropyTweetTrueFromHashtagTrue = calcHashtagProb(groupNum, true, partNum, tweetCount);
            results2 = sqlContext.createDataFrame(condEntropyTweetTrueFromHashtagTrue.coalesce(numPart), new StructType(fields));
            results2.registerTempTable("condEntropyTweetTrueFromHashtagTrue");

            fromresults2 = results2.join(fromHashtagProb, fromHashtagProb.col("hashtag1").equalTo(results2.col("hashtag"))).drop("hashtag1");
            res[partNum] = fromresults2.javaRDD().map(new Function<Row, Row>() {
                @Override
                public Row call(Row row) throws Exception {
                    if (row.getDouble(1) == 0 || row.getDouble(2) == 0)
                        return RowFactory.create(row.getString(0), 0.0);
